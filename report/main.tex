...
The results for the two runs with \texttt{launchshell} as input is presented in table \ref{table:resultslaunchshell}.
...
write about jecxz error, udis/nasm disagreement, jumps and why they don't work, memory adjustment with more than 1 instruction accessing the same register blablabla. Further development: smarter picking of startingbytes. More than one hep is the sync number is low. Better, more advanced substitution of instructions. Smarter more thorough memory adjustment. Potentially, a memory allocation label could be used between MEPs. Add stuff regarding opaque predicates and functionality to add code for finding debuggers to fool dynamic disassemblers.


\chapter{Discussion and further development}
\section{\texttt{launchshell}}



\section{texttt{writetofile}}
